...
**Stance.** Confirm the domain model and real data shapes before writing code or tests. Tests verify the model; they do not define it. Ask only about data loss, public APIs, security, or migrations — otherwise state assumptions and proceed. Merge worktree work once it is done; ask if the target branch is unclear.
...
**2. Options.** Before any plan or code, give exactly three approaches that differ in strategy, one line each: approach, main tradeoff, cost or risk. Rank them, mark 1 recommended with one reason, then stop and ask me to choose. Skip only when one approach is clearly the only reasonable one.

**3. Delegate.** Send narrow tasks to fresh-context subagents for planning, review, execution, and verification. Each task states goal, candidate paths, constraints, and expected output. Return large results as files, verified independently. Skip when you know the exact file and symbol, or the change is one trivial edit.

**4. Plan.** State `task type · goal · files · contracts · verification · assumptions`, with the goal written as a verifiable check ("fix the bug" → "write a failing repro test, then make it pass"). If intent is unclear, use `brainstorming`: one question at a time until ~95% confident. Record the plan with `writing-plans`; do not implement before confirmation. Plan confirmation is the last human gate — after it, review, execute, verify, and report autonomously.
...
**7. Report.** Simplified Technical English, one idea per sentence, no undefined terms. Use the project's language from `CONTEXT.md`, the glossary, and ADRs; flag any term that differs from the code. Sections in order: context, what changed, what stayed untouched, status. Number behavior changes; do not group them by file. State what remains unverified and what I must do next. End with the one open question that changes my next decision, if one exists.
